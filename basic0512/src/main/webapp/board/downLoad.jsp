...
//js->fileDown(<%= % >)
	...
	//out.print("<br>");
	//아래 경로는 톰캣서버가 바라보는 프로젝트 내의 pds폴더가리킴
	String filePath = "D:\\eclipse\\workspace_jsp\\basic0512\\src\\main\\webapp\\pds"; // 절대경로.	
	//가져온 파일이름을 객체화 시켜줌. - 파일이 있는 물리적인 경로가 필요함.
	File file = new File(filePath,b_file.trim());//trim()는 공백을 제거하는 메소드
	...
	try{
		...
		while((data=(fis.read(b,0,b.length)))!=-1){
			sos.write(b,0,data);//파일에서 읽은 내용을 쓰기처리해줌
		}
		//출력 버퍼를 내보냄.
		sos.flush();		
	}catch(Exception e){
		...
		//사용한 자원은 반납해줌. - 묵시적으로 처리됨 - 명싱적으로 처리하는 것 권장사항 - 시점을 앞당김 - 서버에 대한 부담을 줄임 
		//보안상으로 닫지 않으면 악의적인 목적으로 가로채기, 인터셉트한 후에 위변조를 취함 
