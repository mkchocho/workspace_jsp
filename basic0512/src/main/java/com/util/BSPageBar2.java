...
public class BSPageBar2 {
	Logger logger = Logger.getLogger(BSPageBar2.class);
	//전체레코드 갯수
	 ...
	 * 생성자에서 일어남 → 일관성을 위해서 (동일하게 적용해야지 이랬다저랬다 하면 안되기 때문)
	 ***************************************************************************/
	//이 페이지네이션을 사용하는 페이지마다 몇개씩 보여줄지, 페이지이름, 전체레코드 수, URL 당연히 달라야 하지않나
	//해당 페이지에서 생성자의 파라미터로 받아온 정보는 최소한 같은 주소번지에 대해서는 모두 일치해야 한다 - 이랬다 저랬다 하면 안되고...
	public BSPageBar2(int numPerPage, int totalRecord, int nowPage, String pagePath) {
		this.numPerPage = numPerPage;//한 페이지에 몇 개를 뿌릴 거야?
	...
	//setter메소드 선언
	//페이징처리로 필요한 화면을(Tag)을 그려주는 메소드이다 
	public void setPageBar() {
		//문자열을 조작할 때 원본이 바뀌지 않는 스프링 클래스 대신에 StringBuilder{싱글스레드안전 -속도는 조금 빠름}나 StringBuffer{멀티스레드안전-속도는 조금 느림} 클래스를 사용하자
		...
		if(totalRecord>0) {
			...
			if(nowBlock > 0 ) {                                    //(1-1)*2+(2-1)=1
				...
				pageLink.append("<span aria-hidden='true'>&laquo;</span>");// 화살표 방향 >>
				pageLink.append("</a>");
				pageLink.append("</li>");
			}
			//전체 레코드 수가 65개이고 그 중에서 내가 45번 글을 보고 있을 때 라고 가정한다
			for(int i=0;i<pagePerBlock;i++) {//전체 페이지 블록 수는 3개이다 [123 >>] [<< 456 >>] [<< 7]
				//현재 내가 보고 있는 페이지 블록 일때와
				if(nowBlock*pagePerBlock+i==nowPage) {
					//현재 내가 보고 있는 2번째 페이지 블록에서 5번 페이지를 보고 있다면 5번 숫자에 대해서는 링크가 필요없다
				...
				//주의할 것.
				//for문을 탈출할 때는 break문을 사용할 것.
				if((nowBlock*pagePerBlock)+i+1==totalPage) break;//totalPage랑 같으면 더 갈데가 없어요 2+3+0+1=7, 2*3+1+1=8, 2*3+2+1=9
			...
			if(totalBlock > nowBlock+1) {
				...
				pageLink.append("<span aria-hidden=\"true\">&raquo;</span>"); //화살표 방향이 >>
				pageLink.append("</a>");	
				pageLink.append("</li>");
			}
		}
	...
	//getter메소드 선언
	//
	public String getPageBar() {//getPageBar()의 리턴타입이 String 이니까 print메소드 안에서 호출이 가능하였다
		this.setPageBar();
		//그때마다 달라져야 한다. 인스턴스화 할때마다 달라져야 해 → BSPageBar pb = new BSPageBar();
		return pagination;// 123>> <<456>> <<7
	}
}